...
const HistoryContainer = ({launch}) => {
  const handleClick = () => {
    Linking.canOpenURL(launch.links.article_link).then(supported => {
      if (supported) {
        Linking.openURL(launch.links.article_link);
      } else {
        console.log("Don't know how to open URI: " + launch.links.article_link);
      }
    });
  };
